...
-- ===================================================================================
-- Tip 19: JOIN vs EXISTS vs IN: When we are using another table inorder to filter one table. (Only for Filtering)
SELECT 
	o.OrderID,
	o.Sales
FROM Sales.Orders o
INNER JOIN Sales.Customers c
ON o.CustomerID = c.CustomerID
WHERE c.Country = 'USA'			-- Best Practice
...
WHERE EXISTS (
			  ...
			  AND Country = 'USA'
			  )					-- Best if the Table is large

SELECT o.OrderID, o.Sales
FROM Sales.Orders o
WHERE o.CustomerID IN (SELECT CustomerID
					  FROM Sales.Customers c
					  WHERE c.Country = 'USA') -- Don't use IN Operator
...
FROM Sales.Employees
WHERE Salary < (SELECT AVG(Salary) FROM Sales.Employees)  -- Bad Practice
...
SELECT
	...
	CASE 
		 ...
		 ELSE 'Average'
	END Status
FROM Sales.Employees	-- Good Practice

-- ===================================================================================
-- Tip 21: Avoid datatypes VARCHAR & TEXT if possible

-- ===================================================================================
-- Tip 22: Avoid (MAX) unnecessarily large lengths in data types

-- ===================================================================================
-- Tip 23: Use the NOT NULL Constraint whereever applicable
